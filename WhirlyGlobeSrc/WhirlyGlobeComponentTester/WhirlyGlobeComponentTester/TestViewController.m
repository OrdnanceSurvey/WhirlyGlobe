...
- (void)viewDidLoad {
  ...
  configViewC =
      [[ConfigViewController alloc] initWithNibName:@"ConfigViewController"
                                             bundle:nil];
  ...
  // Note: Debugging
  //    if (globeViewC)
  //        [globeViewC setTiltMinHeight:0.001 maxHeight:0.01 minTilt:1.21771169
  //        maxTilt:0.0];
  ...
  if (globeViewC) {
    ...
    [globeViewC
        animateToPosition:MaplyCoordinateMakeWithDegrees(-122.4192, 37.7793)
                     time:1.0];
  } else {
    mapViewC.height = 1.0;
    [mapViewC
  ...
  // Note: Debugging British National Grid
  if (bngTest) {
    // We have to tweak the extents or we end up locked into place when we zoom
    // out.
    //        [mapViewC setViewExtentsLL:MaplyCoordinateMake(-1000000, -1000000)
    //        ur:MaplyCoordinateMake(2000000, -2000000)];
    ...
    MaplyCoordinate localLondon = [mapViewC.coordSys
        geoToLocal:MaplyCoordinateMakeWithDegrees(-0.1275, 51.507222)];
  ...
  if (startupMapType == MaplyGlobeWithElevation) {
    ...
    if (globeViewC)
      [globeViewC setTiltMinHeight:0.001
                         maxHeight:0.04
                           minTilt:1.40
                           maxTilt:0.0];
    ...
    MaplyRemoteTileElevationCesiumSource *cesiumElev = [
        [MaplyRemoteTileElevationCesiumSource alloc]
        initWithBaseURL:@"http://cesiumjs.org/stk-terrain/tilesets/world/tiles/"
                    ext:@"terrain"
                minZoom:0
                maxZoom:16];
    elevSource = cesiumElev;
    cesiumElev.cacheDir =
        [NSString stringWithFormat:@"%@/cesiumElev/", cacheDir];
    ...
    MaplyCesiumCoordSystem *cesiumCoordSys =
        [[MaplyCesiumCoordSystem alloc] init];
    MaplyAnimationTestTileSource *tileSource =
        [[MaplyAnimationTestTileSource alloc] initWithCoordSys:cesiumCoordSys
                                                       minZoom:1
                                                       maxZoom:16
                                                         depth:1];
    ...
    MaplyQuadImageTilesLayer *layer = [[MaplyQuadImageTilesLayer alloc]
        initWithCoordSystem:tileSource.coordSys
    ...
    //        mapViewC.height = 1.0;
    //        [mapViewC
    //        animateToPosition:MaplyCoordinateMakeWithDegrees(86.925278,
    //        27.988056) time:1.0];
  ...
  // Note: Testing
  //    [self performSelector:@selector(findHeightTest) withObject:nil
  //    afterDelay:0.0];
  ...
  [self performSelector:@selector(changeMapContents)
             withObject:nil
             afterDelay:0.0];
  ...
  // Test animation
  //    if (globeViewC)
  //        [self performSelector:@selector(viewAnimationTest) withObject:nil
  //        afterDelay:2.0];

  //    [self performSelector:@selector(labelMarkerTest:) withObject:@(0.1)
  //    afterDelay:0.1];
...
- (void)billboardTest {
  ...
  [screenObj addImage:[UIImage imageNamed:@"veins_diffuse.png"]
  ...
  [baseViewC addBillboards:@[ bboard ]
                      desc:@{
...
- (void)markerOverlapTest {
  ...
  for (unsigned int ii = 0; ii < numTestMarkers; ii++) {
    ...
    MaplyTexture *tex =
        [baseViewC addTextureToAtlas:image mode:MaplyThreadCurrent];
  ...
  [baseViewC addScreenMarkers:markers
                         desc:@{
                           kMaplyClusterGroup : @(0)
                         }
                         mode:MaplyThreadCurrent];
}
...
- (void)markerTest2 {
  ...
  [baseViewC addScreenMarkers:@[ marker1 ]
                         desc:@{
                           kMaplyClusterGroup : @(0)
                         }
                         mode:MaplyThreadAny];
  ...
  [baseViewC addScreenMarkers:@[ marker2 ]
                         desc:@{
                           kMaplyClusterGroup : @(0)
                         }
                         mode:MaplyThreadAny];
...
- (void)labelMarkerTest:(NSNumber *)time {
  ...
  //    [newObjs addObject:[baseViewC addScreenMarkers:redMarkers
  //    desc:@{kMaplyDrawPriority: @(100)} mode:MaplyThreadCurrent]];
  //    [newObjs addObject:[baseViewC addScreenMarkers:blueMarkers
  //    desc:@{kMaplyDrawPriority: @(101)} mode:MaplyThreadCurrent]];
  [newObjs addObject:[baseViewC addScreenLabels:labels
                                           desc:@{
  ...
  //    [self performSelector:@selector(labelMarkerTest:) withObject:time
  //    afterDelay:[time floatValue]];
}
...
- (void)findHeightTest {
  if (globeViewC) {
    ...
    MaplyCoordinate center =
        MaplyCoordinateMakeWithDegrees((7.05090689853 + 8.06813647023) / 2,
...
- (void)viewAnimationTest {
  [globeViewC
      animateToPosition:MaplyCoordinateMakeWithDegrees(-0.1275, 51.507222)
                 height:0.1
                heading:45.0 / 180.0 * M_PI
                   time:10.0];
}
...
- (void)labelExercise {
  ...
  if (mapViewC) {
    ...
    _labelAnimationTimer = [NSTimer
        scheduledTimerWithTimeInterval:1.25
                                target:self
                              selector:@selector(labelAnimationCallback)
                              userInfo:nil
                               repeats:NO];
  }
}
...
- (void)labelAnimationCallback {
  ...
  for (int i = 0; i < 50; i++) {
    ...
    trafficLabelCompObj =
        [mapViewC addScreenLabels:[NSArray arrayWithObjects:label, nil]
                             desc:labelsDesc];
    _trafficLabels[@(i)] = trafficLabelCompObj;
  }

  _labelAnimationTimer =
  ...
  [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation,
    ...
    [self startWMSLayerBaseURL:baseURL
      ...
      failure:^(AFHTTPRequestOperation *operation, NSError *error){
          ...
          //        if (![self startWMSLayerBaseURL:baseURL xml:XMLDocument
          //        layer:layerName style:styleName cacheDir:thisCacheDir
          //        ovlName:ovlName])
          //            NSLog(@"Failed to get capabilities from WMS server:
          //            %@",capabilitiesURL);
      }];
...
- (bool)startWMSLayerBaseURL:(NSString *)baseURL
                         xml:(DDXMLDocument *)XMLDocument
                       layer:(NSString *)layerName
                       style:(NSString *)styleName
                    cacheDir:(NSString *)thisCacheDir
                     ovlName:(NSString *)ovlName {
  // See what the service can provide
  MaplyWMSCapabilities *cap =
      [[MaplyWMSCapabilities alloc] initWithXML:XMLDocument];
  ...
  if (layer && coordSys) {
    MaplyWMSTileSource *tileSource =
        [[MaplyWMSTileSource alloc] initWithBaseURL:baseURL
                                       capabilities:cap
                                              layer:layer
                                              style:style
                                           coordSys:coordSys
    ...
    MaplyQuadImageTilesLayer *imageLayer =
        [[MaplyQuadImageTilesLayer alloc] initWithCoordSystem:coordSys
                                                   tileSource:tileSource];
...
- (void)viewWillAppear:(BOOL)animated {
  // This tests heading
  //    [self performSelector:@selector(changeHeading:) withObject:@(0.0)
  //    afterDelay:1.0];
}

- (void)viewWillDisappear:(BOOL)animated {
  [NSObject cancelPreviousPerformRequestsWithTarget:self
                                           selector:@selector(changeHeading:)
                                             object:nil];
}
...
- (void)changeHeading:(NSNumber *)heading {
  ...
  [self performSelector:@selector(changeHeading:)
             withObject:@([heading floatValue] + 1.0 / 180.0 * M_PI)
             afterDelay:1.0];
}

// Add screen (2D) markers at all our locations
- (void)addScreenMarkers:(LocationInfo *)locations
                     len:(int)len
                  stride:(int)stride
                  offset:(int)offset {
...
// Add 3D markers
- (void)addMarkers:(LocationInfo *)locations
               len:(int)len
            stride:(int)stride
            offset:(int)offset {
...
- (void)addScreenLabels:(LocationInfo *)locations
                    len:(int)len
                 stride:(int)stride
                 offset:(int)offset {
  NSMutableArray *labels = [NSMutableArray array];
...
// Add 3D labels
- (void)addLabels:(LocationInfo *)locations
              len:(int)len
           stride:(int)stride
           offset:(int)offset {
...
// Add cylinders
- (void)addShapeCylinders:(LocationInfo *)locations
                      len:(int)len
                   stride:(int)stride
                   offset:(int)offset
  ...
  for (unsigned int ii = offset; ii < len; ii += stride) {
    ...
    cyl.baseCenter =
        MaplyCoordinateMakeWithDegrees(location->lon, location->lat);
    cyl.radius = 0.01;
    cyl.height = 0.06;
    cyl.selectable = true;
    [cyls addObject:cyl];
...
// Add spheres
- (void)addShapeSpheres:(LocationInfo *)locations
                    len:(int)len
                 stride:(int)stride
                 offset:(int)offset
                   desc:(NSDictionary *)desc {
...
// Add great circles
- (void)addGreatCircles:(LocationInfo *)locations
                    len:(int)len
                 stride:(int)stride
                 offset:(int)offset
                   desc:(NSDictionary *)desc {
...
// Add arrows
- (void)addArrows:(LocationInfo *)locations
              len:(int)len
           stride:(int)stride
           offset:(int)offset
             desc:(NSDictionary *)desc {
  ...
  double size = 1;
  double arrowCoords[2 * 7] = {
      -0.25 * size, -0.75 * size, -0.25 * size, 0.25 * size, -0.5 * size,
      0.25 * size,  0.0 * size,   1.0 * size,   0.5 * size,  0.25 * size,
      0.25 * size,  0.25 * size,  0.25 * size,  -0.75 * size};

  MaplyShapeExtruded *exShape =
      [[MaplyShapeExtruded alloc] initWithOutline:arrowCoords numCoordPairs:7];
  exShape.thickness = size * 1.0;
  exShape.height = 0.0;
  exShape.color = [UIColor colorWithRed:0.8 green:0.25 blue:0.25 alpha:1.0];
  // Each shape is about 10km
  //    exShape.transform = [[MaplyMatrix alloc]
  //    initWithScale:10000*1/EarthRadius];
  exShape.scale = 1.0;
  ...
  for (unsigned int ii = offset; ii < len; ii += stride) {
    ...
    MaplyMatrix *orientMat =
        [[MaplyMatrix alloc] initWithYaw:0.0
                                   pitch:0.0
                                    roll:45.0 / 180.0 * M_PI];
    geomInst.transform = [[[MaplyMatrix alloc]
        initWithScale:10000 * 1 / EarthRadius] multiplyWith:orientMat];
...
// Add models
- (void)addModels:(LocationInfo *)locations
              len:(int)len
           stride:(int)stride
           offset:(int)offset
             desc:(NSDictionary *)desc {
  // Load the model
  NSString *fullPath =
      [[NSBundle mainBundle] pathForResource:@"cessna" ofType:@"obj"];
  if (!fullPath)
    return;
  ...
  // We need to scale the models down to display space.  They start out in
  // meters.
  // Note: Changes this to 1000.0/6371000.0 if you can't find the models
  MaplyMatrix *scaleMat =
      [[MaplyMatrix alloc] initWithScale:1000.0 / 6371000.0];
  // Then we need to rotate around the X axis to get the model pointed up
  MaplyMatrix *rotMat = [[MaplyMatrix alloc] initWithAngle:M_PI / 2.0
                                                     axisX:1.0
                                                     axisY:0.0
                                                     axisZ:0.0];
  ...
  for (unsigned int ii = offset; ii < len; ii += stride) {
    LocationInfo *loc = &locations[ii];
    MaplyMovingGeomModelInstance *mInst =
        [[MaplyMovingGeomModelInstance alloc] init];
    mInst.model = model;
    ...
    mInst.endCenter =
        MaplyCoordinate3dMake(loc2d.x + 0.1, loc2d.y + 0.1, 10000);
...
- (void)addLinesLon:(float)lonDelta lat:(float)latDelta color:(UIColor *)color {
  ...
  for (float lon = -180; lon < 180; lon += lonDelta) {
    ...
    MaplyVectorObject *vec =
        [[MaplyVectorObject alloc] initWithLineString:coords
                                            numCoords:3
                                           attributes:nil];
...
- (NSArray *)addWideVectors:(MaplyVectorObject *)vecObj {
  ...
  MaplyComponentObject *screenLines = [baseViewC
      addWideVectors:@[ vecObj ]
                desc:@{
                  kMaplyColor :
                      [UIColor colorWithRed:0.5 green:0.0 blue:0.0 alpha:0.5],
                  kMaplyFade : @(fade),
                  kMaplyVecWidth : @(3.0),
  ...
  MaplyComponentObject *realLines = [baseViewC
      addWideVectors:@[ vecObj ]
                desc:@{
                  kMaplyColor : color,
                  kMaplyFade : @(fade),
  ...
  for (MaplyVectorObject *road in [vecObj splitVectors]) {
    ...
    MaplyCoordinateSystem *coordSys =
        [[MaplySphericalMercator alloc] initWebStandard];
  ...
  labelObj =
      [baseViewC addScreenLabels:labels
                            desc:@{
...
- (void)addShapeFile:(NSString *)shapeFileName {
  ...
  if (!dashedLineTex) {
    MaplyLinearTextureBuilder *lineTexBuilder =
        [[MaplyLinearTextureBuilder alloc] initWithSize:CGSizeMake(8, 8)];
    [lineTexBuilder setPattern:@[ @(4), @(4) ]];
    lineTexBuilder.opacityFunc = MaplyOpacitySin2;
    UIImage *dashedLineImage = [lineTexBuilder makeImage];
    dashedLineTex =
        [baseViewC addTexture:dashedLineImage
                         desc:@{
  ...
  if (!filledLineTex) {
    MaplyLinearTextureBuilder *lineTexBuilder =
        [[MaplyLinearTextureBuilder alloc] initWithSize:CGSizeMake(8, 32)];
    [lineTexBuilder setPattern:@[ @(32) ]];
    lineTexBuilder.opacityFunc = MaplyOpacitySin2;
    UIImage *lineImage = [lineTexBuilder makeImage];
    filledLineTex =
        [baseViewC addTexture:lineImage
                         desc:@{
                           kMaplyTexMinFilter : kMaplyMinFilterLinear,
                           kMaplyTexMagFilter : kMaplyMinFilterLinear,
                           kMaplyTexWrapX : @true,
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
                 ^{
                   ...
                   MaplyVectorDatabase *vecDb = [[MaplyVectorDatabase alloc]
                       initWithShape:shapeFileName];
                   if (vecDb) {
...
- (void)addGeoJson:(NSString *)name {
  ...
  MaplyLinearTextureBuilder *lineTexBuilder =
      [[MaplyLinearTextureBuilder alloc] initWithSize:size];
  ...
  if (path) {
    ...
    MaplyVectorObject *vecObj =
        [[MaplyVectorObject alloc] initWithGeoJSONDictionary:jsonDictionary];
    if (vecObj) {
      [baseViewC
          addWideVectors:@[ vecObj ]
                    desc:@{
                      ...
                      kMaplyFade : @0,
                      kMaplyDrawPriority :
                          @(kMaplyVectorDrawPriorityDefault + 1),
...
- (void)addArcGISQuery:(NSString *)url {
  ...
  [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation,
                                             id responseObject) {
    MaplyVectorObject *vecObj =
        [[MaplyVectorObject alloc] initWithGeoJSON:responseObject];
    if (vecObj) {
      arcGisObj = [baseViewC addVectors:@[ vecObj ]
...
- (void)addStickers:(LocationInfo *)locations
                len:(int)len
             stride:(int)stride
             offset:(int)offset
               desc:(NSDictionary *)desc {
  UIImage *startImage =
      [UIImage imageNamed:@"Smiley_Face_Avatar_by_PixelTwist"];

  NSMutableArray *stickers = [NSMutableArray array];
  for (unsigned int ii = offset; ii < len; ii += stride) {
    LocationInfo *location = &locations[ii];
    MaplySticker *sticker = [[MaplySticker alloc] init];
    // Stickers are sized in geographic (because they're for KML ground
    // overlays).  Bleah.
    sticker.ll = MaplyCoordinateMakeWithDegrees(location->lon, location->lat);
    sticker.ur = MaplyCoordinateMakeWithDegrees(location->lon + 10.0,
                                                location->lat + 10.0);
...
- (void)addCountries:(NSArray *)names stride:(int)stride {
  ...
  if (CountryTextures) {
    ...
    smileTex = [baseViewC addTexture:smileImage
                         imageFormat:MaplyImageUShort5551
                           wrapFlags:MaplyImageWrapX | MaplyImageWrapY
                                mode:MaplyThreadCurrent];
  }
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    for (NSString *name in names) {
      if (ii % stride == 0) {
        NSString *fileName =
            [[NSBundle mainBundle] pathForResource:name ofType:@"geojson"];
        if (fileName) {

          NSData *jsonData = [NSData dataWithContentsOfFile:fileName];
          if (jsonData) {
            MaplyVectorObject *wgVecObj =
                [[MaplyVectorObject alloc] initWithGeoJSON:jsonData];
            ...
            MaplyComponentObject *compObj =
                [baseViewC addVectors:[NSArray arrayWithObject:wgVecObj]
                                 desc:desc];
            ...
            if ([wgVecObj centroid:&center]) {
              ...
              screenLabel.layoutPlacement =
                  kMaplyLayoutRight | kMaplyLayoutAbove | kMaplyLayoutLeft |
                  kMaplyLayoutBelow;
    ...
    // Note: You could lose track of the objects if you turn the countries
    // on/off quickly
    dispatch_async(dispatch_get_main_queue(), ^{
      // Toss in all the labels at once, more efficient
      MaplyComponentObject *autoLabelObj = [baseViewC
          addScreenLabels:locAutoLabels
                     desc:@{
                       kMaplyTextColor : [UIColor colorWithRed:0.85
                                                         green:0.85
                                                          blue:0.85
                                                         alpha:1.0],
...
- (void)addSun {
  ...
  if (UseSunSphere) {
    ...
    bill.center =
        MaplyCoordinate3dMake(centerGeo.x, centerGeo.y, 5.4 * EarthRadius);
    bill.selectable = false;
    bill.screenObj = [[MaplyScreenObject alloc] init];
    UIImage *globeImage = [UIImage imageNamed:@"SunImage"];
    [bill.screenObj addImage:globeImage
                       color:[UIColor whiteColor]
                        size:CGSizeMake(0.9, 0.9)];
    sunObj = [globeViewC
        addBillboards:@[ bill ]
                 desc:@{
                   kMaplyBillboardOrient : kMaplyBillboardOrientEye,
                   kMaplyDrawPriority : @(kMaplySunDrawPriorityDefault)
                 }
                 mode:MaplyThreadAny];
  }
  ...
  if (UseMoonSphere) {
    ...
    [bill.screenObj
        addImage:moonImage
           color:[UIColor colorWithWhite:moon.illuminatedFraction alpha:1.0]
            size:CGSizeMake(0.75, 0.75)];
    moonObj = [globeViewC
        addBillboards:@[ bill ]
                 desc:@{
                   kMaplyBillboardOrient : kMaplyBillboardOrientEye,
                   kMaplyDrawPriority : @(kMaplyMoonDrawPriorityDefault)
                 }
                 mode:MaplyThreadAny];
  }
...
- (UIImage *)randomImage {
  ...
  [[UIColor colorWithRed:drand48() green:drand48() blue:drand48() alpha:1.0]
      setFill];
  ...
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (unsigned int ii = 0; ii < NumMegaMarkers; ii++) {
          MaplyScreenMarker *marker = [[MaplyScreenMarker alloc] init];
          marker.image =
              [markerImages objectAtIndex:random() % NumMegaMarkerImages];
          marker.size = CGSizeMake(16, 16);
          marker.loc = MaplyCoordinateMakeWithDegrees(drand48() * 360 - 180,
                                                      drand48() * 140 - 70);
...
// Make up a large number of markers and add them
- (void)addMarkerCluster:(LocationInfo *)locations
                     num:(int)howMany
              markersPer:(int)markersPer {
  dispatch_async(
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSMutableArray *markers = [NSMutableArray array];
        UIImage *pinImage = [UIImage imageNamed:@"map_pin"];
        ...
        for (unsigned int ii = 0; ii < howMany; ii++) {
          ...
          for (unsigned int jj = 0; jj < howMany; jj++) {
            ...
            marker.loc = MaplyCoordinateMakeWithDegrees(
                location->lon + drand48() * MarkerSpread,
                location->lat + drand48() * MarkerSpread);
            marker.layoutImportance = 1.0;
            marker.userObject =
                [NSString stringWithFormat:@"%s %d", location->name, jj];
            [markers addObject:marker];
          }
        }

        markerClusterObj = [baseViewC addScreenMarkers:markers
                                                  desc:@{
                                                    kMaplyClusterGroup : @(0)
                                                  }
                                                  mode:MaplyThreadCurrent];
...
- (void)addMarkerPagingTest {
  ...
  markerLayer =
      [[MaplyQuadPagingLayer alloc] initWithCoordSystem:markerDelegate.coordSys
...
- (void)markerSpamRefresh {
  ...
  if (markerLayer)
    [self performSelector:@selector(markerSpamRefresh)
               withObject:nil
               afterDelay:4.0];
}
...
- (void)addAnimatedSphere {
  animSphere = [[AnimatedSphere alloc] initWithPeriod:20.0
                                               radius:0.01
...
- (void)zoomTest {
  ...
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)((2.0 + 1.0) * NSEC_PER_SEC)),
      dispatch_get_main_queue(), ^{
        [mapViewC setHeight:mapViewC.height / 2.0];
      });
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)((2.0 + 2.0) * NSEC_PER_SEC)),
      dispatch_get_main_queue(), ^{
        [mapViewC setHeight:mapViewC.height / 2.0];
      });
  //    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)((2.0+3.0) *
...
- (NSString *)URLForSRS:(NSString *)srs
  ...
  if ([service isEqualToString:@"wmts"]) {
    ...
    [query enumerateObjectsUsingBlock:^(NSURLQueryItem *_Nonnull obj,
                                        NSUInteger idx, BOOL *_Nonnull stop) {
      if (queryString.length != 0) {
        queryString = [queryString stringByAppendingString:@"&"];
      }
      queryString = [queryString
          stringByAppendingString:[NSString stringWithFormat:@"%@=%@", obj.name,
                                                             obj.value]];
    }];
    ...
    baseURL = [baseURL stringByAppendingPathComponent:
                           [srs stringByAddingPercentEscapesUsingEncoding:
                                    NSUTF8StringEncoding]];
    baseURL = [baseURL stringByAppendingPathComponent:
                           [layer stringByAddingPercentEscapesUsingEncoding:
                                      NSUTF8StringEncoding]];
...
- (MaplyCoordinateSystem *)buildBritishNationalGrid:(bool)display {
  ...
  NSString *proj4Str = [NSString
      stringWithFormat:@"+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.9996012717 "
                       @"+x_0=400000 +y_0=-100000 +ellps=airy +nadgrids=%@ "
                       @"+units=m +no_defs",
                       [[NSBundle mainBundle] pathForResource:@"OSTN02_NTv2"
                                                       ofType:@"gsb"]];
  MaplyProj4CoordSystem *coordSys =
      [[MaplyProj4CoordSystem alloc] initWithString:proj4Str];

  // Set the bounding box for validity.  It assumes it can go everywhere by
  // default
  MaplyBoundingBox bbox;
  bbox.ll.x = 1393.0196;
...
- (MaplyCoordinateSystem *)bngForTileSource {
  ...
  // We need to figure out what the real world bounding box of the lowest level
  // (and all the others)
  //  would be if each tile set were a power of two.
  // Formulas courtesy: https://gist.github.com/atlefren/c41921d64a2636c9598e
  double StandardizedRenderingPixelSize =
      0.00028; // A constant having to do with assumed resolution
  int TileWidth = 256,
      TileHeight = 256; // The number of pixels in individual tiles
  int MatrixHeight = 8, MatrixWidth = 8; // The number of tiles at level 0 (e.g.
                                         // level 3) if they were power of 2
  double ScaleDenom =
      3200000.0000000005; // Scale denominator from the Capabilities
  double leftRW = -238375.0,
         topRW = 1376256.0; // Upper left corner from the Capabilities
  double rightRW =
      ScaleDenom * (TileWidth * MatrixWidth) * StandardizedRenderingPixelSize +
      leftRW;
  double bottomRW =
      topRW -
      ScaleDenom * (TileHeight * MatrixHeight) * StandardizedRenderingPixelSize;
...
- (MaplyViewControllerLayer *)configureOSMap {
  ...
  //    initWithBaseURL:[self URLForSRS:@"EPSG:900913" layer:@"Zoom Map 3857"
  //    service:@"wmts"] ext:nil minZoom:7 maxZoom:22];

  MaplyCoordinateSystem *bng = [self bngForTileSource];
  // Note: There's a problem with zoom map auto.  Some of the tiles are simply
  // wrong
  //    NSString *mapName = @"Zoom Map Auto";
  NSString *mapName = @"Zoom Map Tactical 27700";
  MaplyRemoteTileInfo *tileInfo =
      [[OSRemoteTileInfo alloc] initWithBaseURL:[self URLForSRS:@"EPSG:27700"
                                                          layer:mapName
                                                        service:@"wmts"]
                                            ext:nil
                                        minZoom:3
  ...
  layer.drawPriority = BaseEarthPriority;
  // Number of tiles we'll fetch all at once
  layer.numSimultaneousFetches = 8;
  // Normally we'd try to match the screen resolution with tiles, but that seems
  // to overfetch
  //  OS Maps, so we'll back off a bit.  Smaller number for even more backing
  //  off
  layer.importanceScale = 0.5;
  if (startupMapType == Maply2DMap) {
    layer.singleLevelLoading = true;
    // We'll load a level 3 levels back first, which gives us something
    //  to look at while the right level is loading
...
- (void)setupBaseLayer:(NSDictionary *)baseSettings {
...
#ifdef RELOADTEST
  [NSObject cancelPreviousPerformRequestsWithTarget:self
                                           selector:@selector(reloadLayer:)
                                             object:nil];
#endif

  if ([baseLayerName isEqualToString:OSBaseMap]) {
    baseLayer = [self configureOSMap];
    [baseViewC addLayer:baseLayer];
  ...
  if (![baseLayerName compare:kMaplyTestGeographyClass]) {
...
#ifdef RELOADTEST
    [self performSelector:@selector(reloadLayer:)
               withObject:nil
               afterDelay:10.0];
#endif
    ...
    if (globeViewC) {
      // This is the static image set, included with the app, built with
      // ImageChopper
      WGViewControllerLayer *layer =
    ...
    if (zoomLimit != 0 && zoomLimit < maxZoom)
      maxZoom = zoomLimit;
    MaplyRemoteTileSource *tileSource = [[MaplyRemoteTileSource alloc]
        initWithBaseURL:@"http://tile.stamen.com/watercolor/"
                    ext:@"png"
                minZoom:0
                maxZoom:maxZoom];
    tileSource.cacheDir = thisCacheDir;
    ...
    if (zoomLimit != 0 && zoomLimit < maxZoom)
      maxZoom = zoomLimit;
    MaplyRemoteTileSource *tileSource = [[MaplyRemoteTileSource alloc]
        initWithBaseURL:@"http://otile1.mqcdn.com/tiles/1.0.0/osm/"
                    ext:@"png"
                minZoom:0
                maxZoom:maxZoom];
    tileSource.cacheDir = thisCacheDir;
    ...
    int minZoom = 1;
    int maxZoom = 8;
    MaplyRemoteTileInfo *tileSource1 = [[MaplyRemoteTileInfo alloc]
        initWithBaseURL:@"http://map1.vis.earthdata.nasa.gov/wmts-webmerc/"
                        @"MODIS_Terra_CorrectedReflectance_TrueColor/default/"
                        @"2015-05-07/GoogleMapsCompatible_Level9/{z}/{y}/{x}"
                    ext:@"jpg"
                minZoom:minZoom
                maxZoom:maxZoom];
    tileSource1.cacheDir =
        [NSString stringWithFormat:@"%@/daytexture-2015-05-07/", cacheDir];
    MaplyRemoteTileInfo *tileSource2 = [[MaplyRemoteTileInfo alloc]
        initWithBaseURL:@"http://map1.vis.earthdata.nasa.gov/wmts-webmerc/"
                        @"VIIRS_CityLights_2012/default/2015-05-07/"
                        @"GoogleMapsCompatible_Level8/{z}/{y}/{x}"
                    ext:@"jpg"
                minZoom:minZoom
                maxZoom:maxZoom];
    tileSource1.cacheDir =
        [NSString stringWithFormat:@"%@/nighttexture-2015-05-07/", cacheDir];

    MaplyMultiplexTileSource *tileSource = [[MaplyMultiplexTileSource alloc]
        initWithSources:@[ tileSource1, tileSource2 ]];

    MaplyQuadImageTilesLayer *layer = [[MaplyQuadImageTilesLayer alloc]
        initWithCoordSystem:tileSource1.coordSys
                 tileSource:tileSource];
    layer.drawPriority = BaseEarthPriority;
    layer.handleEdges = true;
    layer.requireElev = requireElev;
    ...
    vecWidth = 4.0;
    MaplyAnimationTestTileSource *tileSource =
        [[MaplyAnimationTestTileSource alloc]
            initWithCoordSys:[[MaplySphericalMercator alloc] initWebStandard]
                     minZoom:0
                     maxZoom:22
                       depth:1];
    //        MaplyAnimationTestTileSource *tileSource =
    ...
    MaplyPagingVectorTestTileSource *tileSource =
        [[MaplyPagingVectorTestTileSource alloc]
            initWithCoordSys:[[MaplySphericalMercator alloc] initWebStandard]
                     minZoom:0
                     maxZoom:10];
    MaplyQuadPagingLayer *layer =
        [[MaplyQuadPagingLayer alloc] initWithCoordSystem:tileSource.coordSys
                                                 delegate:tileSource];
    ...
    elevSource = [[MaplyRemoteTileElevationCesiumSource alloc]
        initWithBaseURL:@"http://cesiumjs.org/stk-terrain/tilesets/world/tiles/"
                    ext:@"terrain"
                minZoom:0
                maxZoom:maxZoom];
    ...
    vecColor = [UIColor blackColor];
    vecWidth = 4.0;
    MaplyPagingElevationTestTileSource *tileSource =
        [[MaplyPagingElevationTestTileSource alloc]
            initWithCoordSys:[[MaplySphericalMercator alloc] initWebStandard]
                     minZoom:0
                     maxZoom:10
  ...
  if (jsonTileSpec) {
    NSURLRequest *request =
        [NSURLRequest requestWithURL:[NSURL URLWithString:jsonTileSpec]];
    ...
    [operation setCompletionBlockWithSuccess:^(
      ...
      MaplyRemoteTileSource *tileSource =
          [[MaplyRemoteTileSource alloc] initWithTilespec:responseObject];
      tileSource.cacheDir = thisCacheDir;
...
- (void)quadImageFadeTest:(MaplyQuadImageTilesLayer *)layer {
  if (layer) {
    ...
    [self performSelector:@selector(quadImageFadeTest:)
               withObject:layer
               afterDelay:2.0];
  }
}
  ...
  for (NSString *layerName in [baseSettings allKeys]) {
    ...
    if (isOn && !layer) {
      if (![layerName compare:kMaplyTestUSGSOrtho]) {
        ...
        [self fetchWMSLayer:@"http://raster.nationalmap.gov/arcgis/services/"
                            @"Orthoimagery/USGS_EROS_Ortho_NAIP/ImageServer/"
                            @"WMSServer"
                      layer:@"0"
                      style:nil
                   cacheDir:thisCacheDir
        ...
        MaplyRemoteTileSource *tileSource = [[MaplyRemoteTileSource alloc]
            initWithBaseURL:@"http://tile.openweathermap.org/map/precipitation/"
                        ext:@"png"
                    minZoom:0
                    maxZoom:6];
        tileSource.cacheDir = [NSString
            stringWithFormat:@"%@/openweathermap_precipitation/", cacheDir];
        tileSource.tileInfo.cachedFileLifetime =
            3 * 60 * 60; // invalidate OWM data after three hours
        MaplyQuadImageTilesLayer *weatherLayer =
            [[MaplyQuadImageTilesLayer alloc]
                initWithCoordSystem:tileSource.coordSys
                         tileSource:tileSource];
        ...
        for (unsigned int ii = 0; ii < 5; ii++) {
          MaplyRemoteTileInfo *precipTileSource = [[MaplyRemoteTileInfo alloc]
              initWithBaseURL:[NSString stringWithFormat:@"http://"
                                                         @"a.tiles.mapbox.com/"
                                                         @"v3/"
                                                         @"mousebird.precip-"
                                                         @"example-layer%d/",
                      ...
                      minZoom:0
                      maxZoom:6];
          precipTileSource.cacheDir =
              [NSString stringWithFormat:@"%@/forecast_io_weather_layer%d/",
                                         cacheDir, ii];
          [tileSources addObject:precipTileSource];
        }
        MaplyMultiplexTileSource *precipTileSource =
            [[MaplyMultiplexTileSource alloc] initWithSources:tileSources];
        // Create a precipitation layer that animates
        MaplyQuadImageTilesLayer *precipLayer =
            [[MaplyQuadImageTilesLayer alloc]
                initWithCoordSystem:precipTileSource.coordSys
                         tileSource:precipTileSource];
        ...
        precipLayer.shaderProgramName =
            [WeatherShader setupWeatherShader:baseViewC];
        ...
        thisCacheDir =
            [NSString stringWithFormat:@"%@/mapbox-streets-vectiles", cacheDir];
        [MaplyMapnikVectorTiles
            StartRemoteVectorTilesWithTileSpec:
                @"https://a.tiles.mapbox.com/v4/mapbox.mapbox-streets-v6.json"
            // Note: You need your own access token here
            accessToken:@"pk.eyJ1IjoicGV0ZXJxbGl1IiwiYSI6ImpvZmV0UEEifQ._"
                        @"D4bRmVcGfJvo1wjuOpA1g"
            style:@"https://raw.githubusercontent.com/mapbox/mapbox-gl-styles/"
                  @"master/styles/emerald-v8.json"
            ...
            success:^(MaplyMapnikVectorTiles *vecTiles) {
              ...
              // Note: These are set after the MapnikStyleSet has already been
              // initialized
              MaplyMapboxVectorStyleSet *styleSet =
                  (MaplyMapboxVectorStyleSet *)
                      vecTiles.tileParser.styleDelegate;
              ...
              MaplyQuadPagingLayer *pageLayer = [[MaplyQuadPagingLayer alloc]
                  initWithCoordSystem:[[MaplySphericalMercator alloc]
                                          initWebStandard]
                             delegate:vecTiles];
        ...
        //                [MaplyMapnikVectorTiles
        //                StartRemoteVectorTilesWithTileSpec:@"http://a.tiles.mapbox.com/v3/mapbox.mapbox-streets-v4.json"
        //                  style:[[NSBundle mainBundle]
        //                  pathForResource:@"osm-bright" ofType:@"xml"]
        //                  cacheDir:thisCacheDir
        //                     viewC:baseViewC
        //                   success:
        //                         ^(MaplyMapnikVectorTiles *vecTiles)
        //                        {
        //                            // Don't load the lowest levels for the
        //                            globe
        //                            if (globeViewC)
        //                                vecTiles.minZoom = 5;
        //
        //                            // Note: These are set after the
        //                            MapnikStyleSet has already been
        //                            initialized
        //                            MapnikStyleSet *styleSet = (MapnikStyleSet
        //                            *)vecTiles.styleDelegate;
        //                            styleSet.tileStyleSettings.markerImportance
        //                            = 10.0;
        //                            styleSet.tileStyleSettings.fontName =
        //                            @"Gill Sans";
        //
        //                            // Now for the paging layer itself
        //                            MaplyQuadPagingLayer *pageLayer =
        //                            [[MaplyQuadPagingLayer alloc]
        //                            initWithCoordSystem:[[MaplySphericalMercator
        ...
        //                   failure:
        //                         ^(NSError *error){
        //                             NSLog(@"Failed to load Mapnik vector
        //                             tiles because: %@",error);
        //                         }
        //                 ];
        ...
        NSData *styleData =
            [NSData dataWithContentsOfFile:[[NSBundle mainBundle]
                                               pathForResource:@"MapzenGLStyle"
                                                        ofType:@"json"]];

        if (styleData) {
          thisCacheDir =
              [NSString stringWithFormat:@"%@/mapzen-vectiles", cacheDir];
          MapzenSource *mzSource =
              [[MapzenSource alloc] initWithBase:@"http://vector.mapzen.com/osm"
                                          layers:@[ @"all" ]
        ...
        ParticleTileDelegate *partDelegate = [[ParticleTileDelegate alloc]
            initWithURL:@"http://tilesets.s3-website-us-east-1.amazonaws.com/"
                        @"wind_test/{dir}_tiles/{z}/{x}/{y}.png"
                minZoom:2
                maxZoom:5
                  viewC:baseViewC];
        MaplyQuadPagingLayer *layer = [[MaplyQuadPagingLayer alloc]
            initWithCoordSystem:partDelegate.coordSys
        ...
        MaplyViewControllerLayer *layer = [self configureOSMap];
        //                MaplyCoordinateSystem *bngCoordSys = [self
        //                buildBritishNationalGrid:false];
        //                MaplyAnimationTestTileSource *tileSource =
        //                [[MaplyAnimationTestTileSource alloc]
        //                initWithCoordSys:bngCoordSys minZoom:0 maxZoom:22
        //                depth:1];
        //                //        MaplyAnimationTestTileSource *tileSource =
        //                [[MaplyAnimationTestTileSource alloc]
        //                initWithCoordSys:[[MaplySphericalMercator alloc]
        //                initWebStandard] minZoom:0 maxZoom:22 depth:1];
        //                tileSource.pixelsPerSide = 128;
        //                tileSource.transparentMode = true;
        //                MaplyQuadImageTilesLayer *layer =
        //                [[MaplyQuadImageTilesLayer alloc]
        ...
        //                layer.coverPoles = false;
        //
        //                //        layer.color = [UIColor colorWithWhite:0.5
        //                alpha:0.5];
        //                if (startupMapType == Maply2DMap)
        //                {
        //                    layer.useTargetZoomLevel = true;
...
- (void)changeMapContents {
  imageWaitLoad = [configViewC valueForSection:kMaplyTestCategoryInternal
                                           row:kMaplyTestWaitLoad];
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestLabel2D]) {
    if (!screenLabelsObj)
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestSticker]) {
    if (!stickersObj)
      [self addStickers:locations
                    len:NumLocations
                 stride:4
                 offset:2
                   desc:@{
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestShapeCylinder]) {
    if (!shapeCylObj) {
      [self addShapeCylinders:locations
                          len:NumLocations
                       stride:4
                       offset:0
                         desc:@{
                           kMaplyColor : [UIColor colorWithRed:0.0
                                                         green:0.0
                                                          blue:1.0
                                                         alpha:0.8],
                           kMaplyFade : @(1.0),
                           kMaplyDrawPriority : @(1000)
                         }];
    }
  } else {
    if (shapeCylObj) {
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestShapeSphere]) {
    if (!shapeSphereObj) {
      [self addShapeSpheres:locations
                        len:NumLocations
                     stride:4
                     offset:1
                       desc:@{
                         kMaplyColor : [UIColor colorWithRed:1.0
                                                       green:0.0
                                                        blue:0.0
                                                       alpha:0.8],
                         kMaplyFade : @(1.0),
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestShapeGreatCircle]) {
    if (!greatCircleObj) {
      [self addGreatCircles:locations
                        len:NumLocations
                     stride:4
                     offset:2
                       desc:@{
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestShapeArrows]) {
    if (!arrowsObj) {
      [self addArrows:locations
                  len:NumLocations
               stride:4
               offset:2
                 desc:@{
                   kMaplyColor :
                       [UIColor colorWithRed:1.0 green:0.1 blue:0.0 alpha:1.0],
                   kMaplyFade : @(1.0)
                 }];
    }
  } else {
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestArcGIS]) {
    if (!arcGisObj) {
      [self addArcGISQuery:@"http://services.arcgis.com/OfH668nDRN7tbJh0/"
                           @"arcgis/rest/services/SandyNYCEvacMap/"
                           @"FeatureServer/0/"
                           @"query?WHERE=Neighbrhd=%27Rockaways%27&f=pgeojson&"
                           @"outSR=4326"];
    }
  } else {
    if (arcGisObj) {
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryObjects
                               row:kMaplyTestCountry]) {
    // Countries we have geoJSON for
    NSArray *countryArray = [NSArray
        arrayWithObjects:@"ABW", @"AFG", @"AGO", @"AIA", @"ALA", @"ALB", @"AND",
                         @"ARE", @"ARG", @"ARM", @"ASM", @"ATA", @"ATF", @"ATG",
                         @"AUS", @"AUT", @"AZE", @"BDI", @"BEL", @"BEN", @"BES",
                         @"BFA", @"BGD", @"BGR", @"BHR", @"BHS", @"BIH", @"BLM",
                         @"BLR", @"BLZ", @"BMU", @"BOL", @"BRA", @"BRB", @"BRN",
                         @"BTN", @"BVT", @"BWA", @"CAF", @"CAN", @"CCK", @"CHE",
                         @"CHL", @"CHN", @"CIV", @"CMR", @"COD", @"COG", @"COK",
                         @"COL", @"COM", @"CPV", @"CRI", @"CUB", @"CUW", @"CXR",
                         @"CYM", @"CYP", @"CZE", @"DEU", @"DJI", @"DMA", @"DNK",
                         @"DOM", @"DZA", @"ECU", @"EGY", @"ERI", @"ESH", @"ESP",
                         @"EST", @"ETH", @"FIN", @"FJI", @"FLK", @"FRA", @"FRO",
                         @"FSM", @"GAB", @"GBR", @"GEO", @"GGY", @"GHA", @"GIB",
                         @"GIN", @"GLP", @"GMB", @"GNB", @"GNQ", @"GRC", @"GRD",
                         @"GRL", @"GTM", @"GUF", @"GUM", @"GUY", @"HKG", @"HMD",
                         @"HND", @"HRV", @"HTI", @"HUN", @"IDN", @"IMN", @"IND",
                         @"IOT", @"IRL", @"IRN", @"IRQ", @"ISL", @"ISR", @"ITA",
                         @"JAM", @"JEY", @"JOR", @"JPN", @"KAZ", @"KEN", @"KGZ",
                         @"KHM", @"KIR", @"KNA", @"KOR", @"KWT", @"LAO", @"LBN",
                         @"LBR", @"LBY", @"LCA", @"LIE", @"LKA", @"LSO", @"LTU",
                         @"LUX", @"LVA", @"MAC", @"MAF", @"MAR", @"MCO", @"MDA",
                         @"MDG", @"MDV", @"MEX", @"MHL", @"MKD", @"MLI", @"MLT",
                         @"MMR", @"MNE", @"MNG", @"MNP", @"MOZ", @"MRT", @"MSR",
                         @"MTQ", @"MUS", @"MWI", @"MYS", @"MYT", @"NAM", @"NCL",
                         @"NER", @"NFK", @"NGA", @"NIC", @"NIU", @"NLD", @"NOR",
                         @"NPL", @"NRU", @"NZL", @"OMN", @"PAK", @"PAN", @"PCN",
                         @"PER", @"PHL", @"PLW", @"PNG", @"POL", @"PRI", @"PRK",
                         @"PRT", @"PRY", @"PSE", @"PYF", @"QAT", @"REU", @"ROU",
                         @"RUS", @"RWA", @"SAU", @"SDN", @"SEN", @"SGP", @"SGS",
                         @"SHN", @"SJM", @"SLB", @"SLE", @"SLV", @"SMR", @"SOM",
                         @"SPM", @"SRB", @"SSD", @"STP", @"SUR", @"SVK", @"SVN",
                         @"SWE", @"SWZ", @"SXM", @"SYC", @"SYR", @"TCA", @"TCD",
                         @"TGO", @"THA", @"TJK", @"TKL", @"TKM", @"TLS", @"TON",
                         @"TTO", @"TUN", @"TUR", @"TUV", @"TWN", @"TZA", @"UGA",
                         @"UKR", @"UMI", @"URY", @"USA", @"UZB", @"VAT", @"VCT",
                         @"VEN", @"VGB", @"VIR", @"VNM", @"VUT", @"WLF", @"WSM",
                         @"YEM", @"ZAF", @"ZMB", @"ZWE", nil];
  ...
  if ([configViewC valueForSection:kMaplyTestCategoryAnimation
                               row:kMaplyTestAnimateSphere]) {
    if (!animSphere)
  ...
  baseViewC.performanceOutput =
      [configViewC valueForSection:kMaplyTestCategoryInternal
                               row:kMaplyTestPerf];

  if (globeViewC) {
    globeViewC.keepNorthUp =
        [configViewC valueForSection:kMaplyTestCategoryGestures
                                 row:kMaplyTestNorthUp];
    globeViewC.pinchGesture =
        [configViewC valueForSection:kMaplyTestCategoryGestures
                                 row:kMaplyTestPinch];
    globeViewC.rotateGesture =
        [configViewC valueForSection:kMaplyTestCategoryGestures
                                 row:kMaplyTestRotate];
  } else {
    if ([configViewC valueForSection:kMaplyTestCategoryGestures
                                 row:kMaplyTestNorthUp]) {
      mapViewC.heading = 0;
    }
    mapViewC.pinchGesture =
        [configViewC valueForSection:kMaplyTestCategoryGestures
                                 row:kMaplyTestPinch];
    mapViewC.rotateGesture =
        [configViewC valueForSection:kMaplyTestCategoryGestures
                                 row:kMaplyTestRotate];
  }
  ...
  [hintDict
      setObject:[NSNumber
                    numberWithBool:
                        [configViewC valueForSection:kMaplyTestCategoryInternal
                                                 row:kMaplyTestCulling]]
         forKey:kMaplyRenderHintCulling];
...
- (void)showConfig {
  if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
    popControl =
        [[UIPopoverController alloc] initWithContentViewController:configViewC];
    popControl.delegate = self;
    [popControl
        setPopoverContentSize:CGSizeMake(400.0,
                                         4.0 / 5.0 *
                                             self.view.bounds.size.height)];
    [popControl presentPopoverFromRect:CGRectMake(0, 0, 10, 10)
                                inView:self.view
              permittedArrowDirections:UIPopoverArrowDirectionUp
                              animated:YES];
  } else {
    configViewC.navigationItem.hidesBackButton = YES;
    configViewC.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]
...
- (UIView *)makeSelectionView:(NSString *)msg {
  ...
  testLabel.text = msg;
  CGSize textSize = [testLabel.text
      sizeWithAttributes:@{NSFontAttributeName : testLabel.font}];
  testLabel.frame =
      CGRectMake(marginX / 2.0, 0, textSize.width, textSize.height);
  testLabel.opaque = NO;
  backView.layer.cornerRadius = 5.0;
  backView.backgroundColor =
      [UIColor colorWithRed:0.0 green:102 / 255.0 blue:204 / 255.0 alpha:1.0];
  backView.frame = CGRectMake(-(textSize.width) / 2.0, -(textSize.height) / 2.0,
                              textSize.width + marginX, textSize.height);
...
- (void)handleSelection:(id)selectedObjs {
  ...
  bool isMultiple = [selectedObjs isKindOfClass:[NSArray class]] &&
                    [(NSArray *)selectedObjs count] > 1;
  ...
  if (isMultiple) {
    ...
    if ([selectedObjs isKindOfClass:[NSArray class]]) {
      ...
      if ([selArr count] == 0)
        return;
      selectedObj =
          [(MaplySelectedObject *)[selArr objectAtIndex:0] selectedObj];
    } else
      ...
      if ([vecObj centroid:&loc]) {
        ...
        if ([configViewC valueForSection:kMaplyTestCategoryObjects
          ...
          if (!loftPolyDict[name]) {
            MaplyComponentObject *compObj = [baseViewC
                addLoftedPolys:@[ vecObj ]
                           key:nil
                         cache:nil
                          desc:@{
                            kMaplyColor : [UIColor colorWithRed:0.25
                                                          green:0.0
                                                           blue:0.0
                                                          alpha:0.25],
                            ...
                            kMaplyFade : @(0.5),
                            kMaplyDrawPriority :
                                @(kMaplyLoftedPolysDrawPriorityDefault),
  ...
  //    selectedViewTrack.view = [self makeSelectionView:[NSString
  //    stringWithFormat:@"%@: %@",title,subTitle]];
  //    [baseViewC addViewTracker:selectedViewTrack];
  //
  //    [self performSelector:@selector(moveViewTracker:)
  //    withObject:selectedViewTrack afterDelay:1.0];
  if (title) {
...
- (void)moveViewTracker:(MaplyViewTracker *)viewTrack {
  [baseViewC
      moveViewTracker:viewTrack
               moveTo:MaplyCoordinateMakeWithDegrees(-122.4192, 37.7793)];
}
...
- (void)globeViewController:(WhirlyGlobeViewController *)viewC
  ...
  if (globeViewC) {
    ...
    //        if ([globeViewC geoPointFromScreen:CGPointMake(0, 0)
    //        geoCoord:&geoCoord])
...
#pragma mark -
- (UIViewController *_Nullable)maplyViewController:
